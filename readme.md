...
- `ExpandContour`: `true`, `false`. Включает (`true`) или отключает (по умолчанию `false`)
  "расширение" контура оттиска для последующей обработки лазером.
  Применимо для ударных клейм и для резиновых клейм, изготавливаемых методом лазерной гравировки.
  Необходимость обусловлена тем, что зона обработки лазером имеет отличную от нуля ширину, в результате
  чего без предварительной подготовки контура полученное клеймо будет иметь более тонкие элементы, чем
  ожидается.
