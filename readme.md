...
## Пакетная подготовка изображений клейм для лазерной гравировки

Для лазерной гравировки необходим отдельный файл изображения клейма для каждого клейма.
Возможность пакетной генерации файлов обеспечена начиная с версии 2.3.

### Подготовка сценария пакетной генерации файлов изображений клейм для лазерной гравировки

Для каждого заказа необходимо готовить отдельный makefile.
Для подготовки сценария пакетной генерации следует скопировать
[файл примера](stamps/tests/mf/multifiles.mk) следует скопировать в папку
stamps/sources (папку sources необходимо создать) и назвать по имени заказа
...
Приведённый выше фрагмент - для генерации оттисков поверительных клейм,
поверительных фигурных (ромбовидных) клейм, калибровочных клейм.
...
А так же - фигурные клейма месячные на январь 2018 года с индивидуальным знаком №10 и
калибровочные клейма на январь и февраль 2018 года, с шифром клейма "СП",
размером 18 мм, для индивидуальных знаков калибровщика от 3 до 5 включительно.

#### Параметры сценария пакетной генерации оттисков

Описание основных параметров читайте в предыдущем разделе.

##### Разрешение изображений оттисков

Для указания разрешения генерируемых графических файлов в файл (как и в примере выше)
следует включить следующее предложение:

    MFSTAMPSDPI := 1200

Как результат - разрешение генерируемых файлов - 1200 dpi.
По умолчанию - 2400 dpi.
